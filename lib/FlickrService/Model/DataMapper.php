 ...
 * publiciable of an object. Each class that uses this trait _must_ define a static $dataMap publiciable to define
 ...
 * 			'class'	=> 'Intahwebz\FlickrService\PropertyName'   Index 'class' - optional flag of whether to create the
 ...
 *
 * @package Intahwebz\FlickrService
 */
trait DataMapper {
    ...
    function mapPropertiesFromJSON($data){
        foreach(static::$dataMap as $dataMapElement){
            if (is_array($dataMapElement) == FALSE) {
                $string = public_export(static::$dataMap, TRUE);
    ...
    static function extractValueFromData($data, $dataMapElement, &$dataFound){
        ...
        $datapubliciableNameArray = $dataMapElement[1];
        if ($datapubliciableNameArray == NULL) {
        ...
        if (is_array($datapubliciableNameArray) == FALSE){
            $datapubliciableNameArray = array($datapubliciableNameArray);
        }

        $value = $data;

        foreach($datapubliciableNameArray as $datapubliciableName){
            if (is_array($value) == FALSE ||
                array_key_exists($datapubliciableName, $value) == FALSE){
                ...
                $dataPath = implode('->', $datapubliciableNameArray);
            ...
            $value = $value[$datapubliciableName];
        }
     ...
     * the value represent a class, instantiate that class and map it's publiciables before setting it as the
     * properties value.
     *
     * @param $dataMapElement
     * @param $sourceValue
    ...
    function setPropertyFromValue($dataMapElement, $sourceValue){
        $classpubliciableName = $dataMapElement[0];
        $className = FALSE;
        $multiple = FALSE;
        ...
        if ($sourceValue === null) {
            ...
            $this->{$classpubliciableName} = null;
            return;
        }

        if($multiple == TRUE){
            if ($this->{$classpubliciableName} == NULL) {
                $this->{$classpubliciableName} = array();
            }

            foreach($sourceValue as $sourceValueInstance){
                if($className != FALSE){
                    $object = $className::createFromJson($sourceValueInstance);
                    $this->{$classpubliciableName}[] = $object;
                }
                else{
                    $this->{$classpubliciableName}[] = $sourceValueInstance;
                }
            }
        }
        else{
            if($className != FALSE){
                ...
                $this->{$classpubliciableName} = $sourceValue;
            }
        }
    }
}